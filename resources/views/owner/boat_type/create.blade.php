...
<div class="d-flex align-items-center mb-3">
    <div>
        <ul class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ url('owner/boat_types') }}">{{__('owner.boat_type.title')}}</a></li>
            <li class="breadcrumb-item active">{{__('owner.boat_type.create.title')}}</li>
        </ul>
        <h1 class="page-header mb-0">{{__('owner.boat_type.create.title')}}</h1>
    </div>
</div>
<div id="formControls" class="mb-5">
    <div class="card">
        <div class="card-body pb-2">
            <form action="{{ route('owner.boat_types.store') }}" method="post" id="createForm" enctype="multipart/form-data">
                @csrf
                <div class="row mb-3">
                    <div class="col-xl-3">
                        <div class="form-group">
                            <label for="name_ar" class="form-label">{{ __('owner.boat_type.name') }} <span class="text-danger">*</span></label>
                            <input type="text" name="name_ar" value="{{ old('name_ar') }}" class="form-control" required placeholder="{{ __('owner.boat_type.name') }}">
                    ...
                    <div class="col-xl-3">
                        <div class="form-group">
                            ...
                            <input type="text" name="name_en" value="{{ old('name_en') }}" class="form-control" required placeholder="{{ __('owner.boat_type.name_en') }}">
                            @error('name_en') <span class="text-danger error">{{ $message }}</span>@enderror
                        </div>
                    </div>

                    <div class="col-md-3">
                        <label for="status" class="form-label">{{ __('owner.boat_type.status') }} <span class="text-danger">*</span></label>
                        <select name="status" required class="form-control">
                            <option value="1" {{ old('status') == 1 ? 'selected' : '' }}>{{ __('owner.boat_type.status_active') }}</option>
                            <option value="0" {{ old('status') == 0 ? 'selected' : '' }}>{{ __('owner.boat_type.status_inactive') }}</option>
                        </select>
